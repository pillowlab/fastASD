function [kest,ASDstats,dd] = fastASD_wellcond(x,y,dims,minlens,nxcirc,adddiag)
% Automatic smoothness determination (ASD) for nD filter w/ isotropic smoothness
%
% [kest,ASDstats] = fastASD(x,y,dims,minlens,nxcirc,adddiag)
%
...
%     nxcirc [2 x 1] - circular boundary for each dimension (OPTIONAL)  [Default = max(dims)*1.25]
...
if (nargin < 6) || isempty(adddiag)
    adddiag = 1e-6; % default value for additive constant to diagonal
end
...
lfun = @(prs)neglogev_ASDspectral_wellcond(prs,dd,wwnrmtot,adddiag);  % loss function (note no condthresh used)
