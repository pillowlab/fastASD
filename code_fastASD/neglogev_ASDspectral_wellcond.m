function [neglogev,grad,H,mupost,Lpost,ii,Cinv] = neglogev_ASDspectral_wellcond(prs,dd,wwnrm,adddiag)
% Neg log-evidence for ASD regression model in Fourier domain without
% truncating eigenvalues
%
% [neglogev,grad,H,mupost,Lpost,ii,Cinv] = neglogev_ASDspectral_wellcond(prs,dd,wwnrm,condthresh)
%
...
%    adddiag [1 x 1] - constant to be added to cdiag to ensure prior covariance is well-conditioned
%
% OUTPUT:
% -------
...
switch nargout 
    case {0,1} % compute just diagonal of C
        cdiag = mkcovdiag_ASDstd_wellcond(len,trho,wwnrm,adddiag); % compute diagonal and Fourier freqs

    case 2
        % compute diagonal of C and deriv of C^-1
        [cdiag,dcinv] = mkcovdiag_ASDstd_wellcond(len,trho,wwnrm,adddiag); % compute diagonal and Fourier freqs

    otherwise
        % compute diagonal of C, 1st and 2nd derivs of C^-1 and C
        [cdiag,dcinv,dc,ddcinv] = mkcovdiag_ASDstd_wellcond(len,trho,wwnrm,adddiag); % compute diagonal and Fourier freqs
end
