function [neglogev,grad,H,mupost,Lpost,ii] = neglogev_ASDspectral_nD(prs,dd,wwnrm,condthresh)
...
% INPUT:
% -------
%      prs [d+2 x 1] - ASD parameters [rho; len (for each dimension); nsevar]
...
%     wwnrm  [m x d] - vector of normalized DFT frequencies, along each dimension
...
ii = wwnrm*lens.^2 < 2*log(condthresh); % frequency coeffs to keep
...
switch nargout 
    case {0,1} % compute just diagonal of C
        cdiag = mkcovdiag_ASDstd_nD(trho,lens,wwnrm(ii,:)); % compute diagonal and Fourier freqs

    case 2
        % compute diagonal of C and deriv of C^-1
        [cdiag,dcinv] = mkcovdiag_ASDstd_nD(trho,lens,wwnrm(ii,:)); % compute diagonal and Fourier freqs

    otherwise
        % compute diagonal of C, 1st and 2nd derivs of C^-1 and C
        [cdiag,dcinv,dc,ddcinv] = mkcovdiag_ASDstd_nD(trho,lens,wwnrm(ii,:)); % compute diagonal and Fourier freqs
end
...
if nargout >= 3 
    ...
    nthet = nl+1;  % number of theta variables (rho and len)
